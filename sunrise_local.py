...
def handle_other(message):
    ...
    #         print("exists")
    # # print(str("♚Amirali♚") in users_list.)
    # for i in users_list:
    #     if message.from_user.first_name and message.from_user.username not in users_list:
    #         users_list.append({str(message.from_user.first_name): str(message.from_user.username)})
    # print(users_list)
    #
    ...
    if user_text == 'Change Time':
        ...
        bot.send_message(chat_id, "Example : 2020/9/16")
    # if str(user_text).count(' ') or str(user_text).count('/') == 2:
    #     change_time(message)
    # if is_date(user_text):
    #     change_time(message)
